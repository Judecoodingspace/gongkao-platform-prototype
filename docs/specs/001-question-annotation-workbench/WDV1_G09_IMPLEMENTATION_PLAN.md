...
```text
PLAN_STATUS = CORRECTIVE REVISION 1 — READY FOR TEAM B FINAL REVIEW
PLAN_REVISION = 1
...
This document is an implementation plan only. Corrective Revision 1 decomposes Contract Rev3 into reviewable backend work packages and resolves the five remaining Team B Plan findings, but it does not authorize a migration, Python change, test implementation, frontend work, or merge. The future implementation must stop after each checkpoint in section 31 until the applicable review is complete.
...
|---|---|---|
| Governance / Contract, `Judecoodingspace/gongkao-platform-prototype` | `f0668710456f4e9ea81014407ff5134e61bcc448` | the exact object and `origin/docs/20260916-wdv1-g09-annotation-e2e-contract` resolved to the required SHA; worktree clean before the Rev1 branch was created |
...
Previous Plan source: `WDV1_G09_IMPLEMENTATION_PLAN.md` at `29f3225df78b403a42ccf962185ffa74ed741ed0` on `plan/20260917-wdv1-g09-annotation-e2e`. Rev1 branch: `plan/20260919-wdv1-g09-annotation-e2e-rev1`, created directly from Contract Rev3 `f0668710456f4e9ea81014407ff5134e61bcc448` without merging the old Plan branch. The Contract and Frozen Decisions remain read-only.

## 3. Authority

Implementation authority order is fixed:

1. `WDV1_G09_PRODUCT_FROZEN_DECISIONS.md` as contained in the Contract Rev3 lineage;
2. `WDV1_G09_IMPLEMENTATION_CONTRACT.md` at `f0668710456f4e9ea81014407ff5134e61bcc448`;
...
G-09 replay is effect-idempotent: the same actor, scope, `client_request_id`, and fingerprint never execute the business mutation twice, while a replay may return the current safe authoritative resource representation after later legal changes. It does not promise a byte-for-byte historical HTTP response. Stable result identity remains mandatory where relevant, including package import, question creation, and save-and-next. Package-owned current questions also maintain the invariant `AnnotationPackageQuestion.knowledge_point_id == QuestionVersion.knowledge_point_id`; ordinary field or knowledge-point save updates both mirrors in one transaction without changing source-topic metadata, provenance, or `package.row_version`.
...
| Concurrency/idempotency | package lock serializes tail allocation; DB unique constraints are final guard; stale package gets `STALE_PACKAGE`; replay resolves the same question/version identity without creating another question and may return its current safe representation. |
...
| Concurrency/idempotency | scope `annotation.question.patch:{version_id}`; identical replay does not mutate or increment any row version again and may return the current safe question representation; stale returns current safe tokens; no automatic merge/overwrite. |
| Tests | independent round trips and LF-only normalization; autosave replay/conflict/two-tab stale; knowledge-point A→B updates QuestionDraft, membership, package list/filter while leaving source topic/provenance/package token unchanged and increments the question token once; last-edited only on successful mutation; generic create service/API bypass tests reject both package source roles with no Question/Version/Slot, while standalone PaperVersion and package-route creation still work; all other generic bypass attempts and non-package regressions. |
...
| Concurrency/idempotency | no package-token churn; row-version winner only; fingerprint contains field/mode/confirmation/ordered result+block IDs/expected token, never source text; replay never repeats fill, append, or replace and may return the current safe question representation. |
| Tests | empty fill; non-empty fill reject; append exact LF and copied provenance; replace confirmation/history; multi-role/multi-block deterministic lock order; stale active selection; cross-package/result/block; duplicate/empty input; manual edit retains head; history survives active switch; source action racing active-result adoption and source action racing completion prove no deadlock and no stale source is silently accepted. |
...
| Concurrency/idempotency | replay resolves the same originally selected/created next-question identity and never creates a second next question; returned DTOs may reflect later legal state. Package lock serializes different commands; stale loser receives `STALE_PACKAGE`/`STALE_DRAFT`; unique global/local order is final guard. |
...
| Service | resume chooses last-edited current question; if in-progress pointer null use first existing question; not-started returns null; completed marks read-only; response includes fields, provenance, workspace and both tokens. Workspace stores only source view. `PATCH /workspace` unconditionally returns `409 ANNOTATION_COMPLETED_READ_ONLY` when the package is completed; only explicit reopen restores workspace mutation. |
...
| Tests | last edited not last viewed; not-started/in-progress fallback; creator vs assigned separate workspace; stale/replay; temporary selection/popup/dirty/scroll absent; completed resume GET succeeds, completed workspace PATCH is rejected, reopen then workspace PATCH succeeds. |
...
| 22 | `tests/services/test_annotation_package_service.py` | ADD | 3,5–7,11–13 | intake, mandatory initial-processing boundary, reads, creation/save/KP mirror/generic-create guard/resume/completion/reopen |
...
| 25 | `tests/services/test_annotation_concurrency.py` | ADD | 2,6–13 | SAVEPOINT races, stale tokens, source action vs adoption/completion, save-next, truthful source-topic replay, completion races |
...
| 28 | `tests/api/test_annotation_workbench_contract.py` | ADD | 6–14 | question/save/action/next/topic/resume/workspace/completed guard/generic-create bypass/complete/reopen routes |
...
| create question | `annotation.question.create:{package_id}` | package token, knowledge point, source-topic metadata | same question/version identity; never create another question; current safe representation allowed |
...
| save-and-next | `annotation.question.save_next:{version_id}` | both tokens and optional current field patch | preserve originally selected/created next-question identity; never create a second next question; current DTO state allowed |
...
| 5. Annotation/provenance service | package/provenance service tests | dynamic creation; four fields; KP mirror; autosave; fill/append/replace; multi-block/role lock ordering; exact links; manual edit head preservation; history; save-next; first/replayed source-topic correction; completion/reopen |
| 6. Router/API contract | two new annotation API files plus existing error/Shenlun tests | all 20 routes; status/replay headers; forbidden/not-found; DTO extra forbid; cursor; error envelope/tokens; generic package-bound create rejection/non-package compatibility; completed workspace protection; OpenAPI presence |
| 7. Concurrency/idempotency | `test_annotation_concurrency.py` | simultaneous first-key reservation; create tail; autosave; append retry; source action vs adoption/completion; save-next double-click/two sessions; source-topic first success/immediate replay/replay after later mutation; completion races; reopen; same ID/different fingerprint |
...
| 9. Manual E2E | section 32 | synthetic dual DOCX from one import action through automatic initial-processing command and reopen, including failures, adoption, KP mirror, generic-create guard, source-action races, truthful correction replay, completed workspace guard, warnings and task-list return |
...
| source-action canonical lock order | 8 | package/source/question/provenance repositories and service | action-vs-adoption/completion no-deadlock and stale-selection tests |
...
| source-topic-local ordering | 1,10 | M-007/repository/service | IDs 16–16d,26 |
...
Protected baseline capabilities are: G-07 Paper/PaperVersion intake and private storage; standalone non-package Question creation/patch/submission/correction/versioning; QuestionSlot ordering; five Shenlun knowledge points and subject/type validation; generic API compatibility except the required package-bound PaperVersion create guard; the entire M-006 Processing Service, activation, idempotency, immutable history, parser, stale classification, and read side; PostgreSQL test guards; and all G-07/G-08 behaviors. Regression evidence must additionally prove both knowledge-point mirrors remain equal, source-action/adoption/completion use one lock order, and M-007 remains exactly seven tables with no response receipt storage.
...
| 6 | `feat(annotation): add dynamic question creation and draft save` | create/autosave/KP mirror/last-edited plus package-source generic-create guard |
| 7 | `feat(annotation): add exact source field provenance` | canonical source lock order, fill/append/replace/history transaction |
...
| C — Annotation core/provenance | 6–9 | existing Shenlun regressions, KP mirror, generic-create guard, source-action lock races, autosave/provenance/save-next/first+replay reorder concurrency | canonical source, exact provenance, effect idempotency, one lock order, generic bypass closure, truthful replay | any stale overwrite/source acceptance, duplicate next/order, mirror divergence, or unverifiable provenance |
...
| E — Full E2E candidate | 12 plus all prior | full suite/static/migration/OpenAPI/manual report, 36-ID traceability proof, no-receipt schema proof, and clean diff | acceptance traceability, protected regressions, operational reproducibility | any unmapped acceptance item, failing regression, or scope drift |
...
10. Correct question 2 source topic; prove first success leaves knowledge point unchanged, makes old/new groups contiguous by global order, returns committed affected rows/tokens and `ordering_refresh_required=false`. Record tokens/rows, perform another legal mutation, then retry the original request ID/payload; prove no reorder or row-version increment runs, `Idempotent-Replay: true`, current target/package tokens, `affected_questions=null`, and `ordering_refresh_required=true`, then GET the question list to recover current ordering. Reuse the ID with changed metadata and confirm `IDEMPOTENCY_CONFLICT`.
...
12. Attempt completion with missing fields, then observe all soft warnings, confirm exact stable warning set, reach completed, return to task list without auto-opening another package, prove resume GET still works, and prove every annotation/generic write including workspace PATCH is blocked.
13. Reopen explicitly, prove workspace PATCH and content edit now succeed, inspect safe audit/log/error/idempotency records and schema, confirm no source/answer/document bytes/HTTP body/affected-row JSON/private path/parser output/traceback or replay-receipt storage, and prove no submit/review/approval/publication record exists.
...
All internal choices required for execution are fixed here: module name; single seven-table M-007 revision; exact table/constraint decomposition; policy mapping for the existing actor model; cursor payload; repository/service boundary; effect-idempotency without response receipts; truthful source-topic replay; knowledge-point mirror transaction; canonical source-action lock order; generic package-source create guard; mandatory same-flow initial-processing orchestration boundary; unconditional completed workspace protection; files, tests, commits, and checkpoints.
...
```text
CONTRACT_REV3_SHA_VERIFIED = YES
BACKEND_BASELINE_SHA_VERIFIED = YES

PLAN_BRANCH_DESCENDS_FROM_CONTRACT_REV3 = YES
...
REV3_EFFECT_IDEMPOTENCY_PROPAGATED = YES
REV3_SOURCE_TOPIC_REPLAY_PROPAGATED = YES
REV3_NO_RESPONSE_RECEIPT_SCHEMA = YES

CONTRACT_ACCEPTANCE_IDENTIFIER_COUNT = 36
ALL_36_ACCEPTANCE_IDENTIFIERS_MAPPED = YES
...
PLAN_STATUS = READY_FOR_TEAM_B_FINAL_REVIEW
IMPLEMENTATION_AUTHORIZED = NO
NEXT_STAGE = TEAM B IMPLEMENTATION PLAN REV1 FINAL REVIEW
```
