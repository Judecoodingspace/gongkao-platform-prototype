...
```text
STATUS = APPROVED（TEAM B IMPLEMENTATION PLAN RE-REVIEW #2 PASS）
...
- Governance repo 本地 `main@99433c8`，remote SHA 已验证一致（remote readback SHA-256 match）。
...
| Parser | `src/gongkao_api/modules/source_processing/parser.py` | `parse_docx` | Reuse（bytes-only 调用） |
...
| Unit | 内容 | 落点 |
| --- | --- | --- |
| P1 command / input model | `TriggerProcessing(client_request_id, paper_version_id, parser_name, parser_version, parser_config)`、`ActivateProcessing(client_request_id, processing_result_id)` frozen dataclass；`TriggerProcessingFingerprintPayload(paper_version_id, parser_name, parser_version, parser_config)`（trigger request_hash 输入，见 §6）；`ActivateProcessingFingerprintPayload(processing_result_id)`（activate request_hash 输入，见 §14）；`ProcessingExecutionContext(actor_id, client_request_id, processing_result_id, paper_version_id)`（Transaction A → parser phase → Transaction B 的 bounded 上下文，见 §7/§13）；`ProcessingOutcome`（bounded 终态输入，见 §7.1）；内部编排符号 `_reserve_processing(...)`、`_execute_processing(...)`、`_terminalize(...)`（见 §7.1） | service.py |
...
| P5 Transaction A | 见 §8 | service.py `trigger_processing` |
| P6 source read + integrity | 见 §11 | service.py parser phase |
| P7 parser invocation | `parse_docx(content, parser_config=ParserConfig(...))`（canonical config propagation） | service.py |
| P8 ParseResult → persistence mapper | 见 §13 | service.py `_map_parse_result()` |
| P9 Transaction B | 见 §14 | service.py `_terminalize()` |
| P10 failure mapping / safe diagnostics | 见 §16 | service.py `_failures.py 语义内化于 service` |
| P11 active-selection operations | initial / explicit / read | service.py + repository.py |
| P12 stale/interrupted classifier | 纯函数 + config + 注入 now | service.py |
| P13 reads | result/history/blocks/gaps/active | service.py |
| P14 audit integration | 3 个 action code | service.py + AuditRepository |
| P15 tests / concurrency qualification | 见 §19–§21 | tests/services/test_source_processing_service.py |
...
9. 构建 `ProcessingExecutionContext(actor_id, client_request_id=command.client_request_id, processing_result_id=result_id, paper_version_id=command.paper_version_id)`，随返回交给调用路径（parser phase → Transaction B 使用，见 §13）。
...
trigger_processing(actor_id, command)
    |
    +-- _reserve_processing(actor_id, command)
    |      # Transaction A（§7 步骤 1–10，单事务提交）
    |      → replay / redirect：返回既有 result/status
    |      → 或：返回新 ProcessingExecutionContext
    |
    +-- if replay / redirect:
    |      return 既有 result/status（MUST NOT 重跑 parser）
    |
    +-- outcome = _execute_processing(context)
    |      # 数据库事务之外（Transaction A 提交后才开始）
    |      → 重新加载 PaperVersion 来源 metadata
    |      → source_reader.read(storage_uri)
    |      → service 重算 content SHA-256（§10）
    |      → 完整性 / 一致性检查
    |      → 构建 ParserConfig 并调用 parse_docx(...)
    |      → 产出 bounded ProcessingOutcome
    |
    +-- view = _terminalize(context, outcome)
           # Transaction B（§13，单事务提交）
           → 返回终态 ProcessingResultView / result DTO
```

新增内部符号（service.py 内）：`ProcessingOutcome`（bounded：`result_status`、`blocks`、`gaps`、`diagnostic_code`、`diagnostic_metadata`、`verified_source_hash`）、`_reserve_processing(...)`、`_execute_processing(...)`、`_terminalize(...)`。
...
1. 重新加载 `PaperVersion`（`get_version`）以取得 `storage_uri` 与 `file_hash`（不缓存 metadata 跨重读）。
2. `source_reader.read(paper_version.storage_uri) -> SourceReadResult`：
...
5. **完整性比对**：`actual_sha256 != paper_version.file_hash` → Transaction B failed：`diagnostic_code='SOURCE_INTEGRITY_FAILURE'`，`verified_source_hash=None`，parser MUST NOT 被调用（不匹配的实际 hash 不得写入 `verified_source_hash`）。
...
8. 对应测试：bytes 重算值与 `file_hash` 不一致 → integrity failure；reader 返回 sha 与 content 重算值不一致 → fail-closed internal consistency failure；重算值匹配 → `verified_source_hash = actual_sha256`。不得修改 Source Reader。
...
- 调用形态（保持 service 边界的 canonical 配置身份）：
  ...
  parser_config = ParserConfig(
      ...
      parser_version=reservation.parser_version,
      canonical_config=reservation.parser_config,
  )
...
- **explicit activation**（`activate_processing_result`，scope `source_processing.activate`，沿用幂等模板；`client_request_id` 只作 key 维度，fingerprint payload 为 `ActivateProcessingFingerprintPayload(processing_result_id)`，与 §6 同一 hash 纪律）。**选择 Option A**（命令只含 `processing_result_id`）；**non-locking pre-read 必须位于同一个显式 `with self._session.begin():` 事务内**（不得使用第二个 Session，不得在显式事务外先 SELECT）：

  ```text
  with self._session.begin():

      candidate_hint =
          SourceProcessingRepository.get_result(command.processing_result_id)
      ...
      candidate =
          SourceProcessingRepository.lock_result(command.processing_result_id)
          # lock class 2
      ...
      AuditRepository.append(action_code="source_processing.active_changed",
                             actor_id=actor_id,
...
| R7 来源在 metadata→read 间损坏 | 每次尝试重读重算（§10） | service（fault injection：storage.read 返回篡改内容） | 注入 mismatch | failed + SOURCE_INTEGRITY_FAILURE + verified_source_hash=None + parser 未被调用 |
...
## §A2 Team B final approval record
